...
buildAndTagDockerImage(){
    docker build -t gcr.io/${PROJECT_ID}/app-dir:$CIRCLE_SHA1 .
}
...
publishDockerImage(){
    ...
    docker push gcr.io/${PROJECT_ID}/app-dir:$CIRCLE_SHA1
}

deployToKubernetesCluster(){
    kubectl set image deployment/${DEPLOYMENT_NAME} ${CONTAINER_NAME}=$DOCKER_REG/${PROJECT_ID}/app-dir:$CIRCLE_SHA1
}
